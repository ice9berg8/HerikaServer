...
class openrouterjsonanthropic
{
    ...
    public function open($contextData, $customParms)
    {
        ...
        foreach ($contextDataOrig as $n=>$element) {
            ...
            if ($n>=($n_ctxsize-1) && $element["role"]!="tool") {
                ...
                if ($lastrole == "assistant" && $lastrole != $element["role"] && $element["role"]!="tool" ) {
                    $contextDataCopy[]=[
                        "role"=>"assistant",
                        //ICE -- coverts assistant part of the request into text format. Caching only allow text content.
                        // "content"=>"{\"character\": \"{$GLOBALS["HERIKA_NAME"]}\", \"listener\": \"$lastTargetBuffer\", \"mood\": \"\", \"action\": \"Talk\",\"target\": \"\", \"message\":\"".trim($assistantRoleBuffer)."\"}"
                        "content" => $GLOBALS["HERIKA_NAME"] . " @"." " . $lastTargetBuffer . trim($assistantRoleBuffer)
                ...
                if ($element["role"]=="system") {
                    ...
                    if (isset($element["tool_calls"])) {
                        ...
                        if (is_array($alreadyJs)) {
                            $contextDataCopy[]=[
                                "role"=>"assistant",
                                //ICE -- No need for this since assistant content is a text now
        ...
        //ICE -- Caching util functions
        if(!function_exists('cacheReadFromFile')) {
            function cacheReadFromFile($fileName)
            {
                if(!file_exists(__DIR__."/../temp/".$fileName)){
                    return 0;
                }
                $file = fopen(__DIR__."/../temp/".$fileName, "r") or die("Unable to open file!"); // Use "r" for read mode
                $timestamp = "";
                $content = "";
            
                if (filesize(__DIR__."/../temp/".$fileName)) {
                    $firstLine = fgets($file); // Read first line (timestamp)
                    $timestamp = trim($firstLine); // Remove any extra spaces or newlines
            
                    $remaining = "";
                    while (!feof($file)) {
                        $remaining .= fgets($file);
                    }
        ...
        if(!function_exists("cacheWriteToFile")){
            function cacheWriteToFile($fileName, $content, $timestamp)
            {
        ...
        if(!function_exists("getStringDifference")){
            function getStringDifference($cache, $current) {
                $array1 = explode(' ', $cache);
                $array2 = explode(' ', $current);
                ...
                return [
                    'diff_in_cache' => implode(' ', $diff1),
                    'diff_in_current' => implode(' ', $diff2),
        ...
        // Compacting */
        //ICE -- user and assistant content caching.
        $contextDataCopy = [];
        $finalUserMessages = "";
        ...
        if($cacheUserMessages ==0 || $cacheAssistantMessages == 0 || time()-$cacheUserMessages["timestamp"]>300){
            ...
            foreach ($contextData as $n => $element) {
                ...
                if ($element["role"] == "user" && $n < $n_ctxsize - 4) {
                    $finalUserMessages .= $element["content"] . " ";
            ...
            //ICE -- Write to cahce file if cache not exist or cache is out dated than 5 minutes
            $writeTime = time();
            ...
            foreach ($contextData as $n => $element) {
                if ($element["role"] == "user" && strpos($cacheUserMessages["content"],$element["content"]) ===false) {
                    $contextDataCopy[] = $element;
                } else if ($element["role"] == "assistant" && strpos($cacheAssistantMessages["content"],$element["content"])===false) {
                    $contextDataCopy[] = $element;
                } else if ($element["role"] != "user" && $element["role"] != "assistant"){
                    $contextDataCopy[] = $element;
                }
            }
        }
        ...
        if (!$assistantAppearedInhistory) { // is this still needed?
            ...
            // ICE
            // $finalContextDataWithExamples[]=["cache_control" =>["type" => "ephemeral"]];
        ...
        //ICE -- caching system prompt  for 5 mins
        // TODO handle inventory changes/ environemntal changes
        $systemCache = cacheReadFromFile("system_cache_{$GLOBALS["HERIKA_NAME"]}.tmp");
        foreach ($data["messages"] as $key => &$message) {
            if ($message["role"] == "system") {
                if($systemCache == 0 || time() - $systemCache["timestamp"] > 600){
                    ...
                    cacheWriteToFile("system_cache_{$GLOBALS["HERIKA_NAME"]}.tmp",$content,time());

                }else{
                    $message["content"] = [[
                        "type" => "text",
                        "text" => $systemCache["content"],
                        "cache_control" => ["type" => "ephemeral"]
                    ]];
                }
            }
        ...
        //ICE --logging only for context inspection
        /*
        file_put_contents(
            __DIR__ . "/../log/context_sent_to_llm_json.log",
    ...
    public function process()
    {
        ...
        /*
        file_put_contents(
            __DIR__ . "/../log/full_response_from_llm.log",
            date(DATE_ATOM) . "\n=\n" . json_encode($data, JSON_PRETTY_PRINT) . "\n=\n",
            FILE_APPEND
        );*/
